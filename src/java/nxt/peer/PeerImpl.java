...
final class PeerImpl implements Peer {
    ...
    private volatile PeerWebSocket webSocket;
    ...
    PeerImpl(String peerAddress, String announcedAddress) {
        ...
        this.port = Constants.isTestnet ? Peers.TESTNET_PEER_PORT : Peers.myPeerServerPort;
        this.state = State.NON_CONNECTED;
        this.shareAddress = true;
        this.webSocket = new PeerWebSocket();
    }

    void setWebSocket(PeerWebSocket webSocket) {
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            if (Peers.useWebSockets && !webSocket.isOpen()) {  
                useWebSocket = webSocket.startClient(URI.create("ws://" + getHost() + ":" + getPort() + "/burst"));
            }
            ...
            if (useWebSocket && webSocket.isOpen()) {  
                ...
                if (communicationLoggingMask != 0)
                    log = "WebSocket " + getHost() + ": " + wsRequest;
            ...
            else if ((!useWebSocket && Peers.useBothWebSocketsAndHttp) || !Peers.useWebSockets) {
